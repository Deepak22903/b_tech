...
index=0
clientSocket.send(f"0 : {data[index]}".encode('utf-8'))
index+=1
clientSocket.send(f"1 : {data[index]}".encode('utf-8'))
index+=1
clientSocket.send(f"2 : {data[index]}".encode('utf-8'))
...
while True:
    ...
    if window:
        ...
        serverResponse = f"{index%3} : {serverResponse}"
