    ...
    try:
        while True:
            ...
            if message:
                ...
                if "NACK" in message.decode('utf-8'):
                    print("NACK received, resending message")
                    message = data[0]
                    checksum = 0
                    for i in message:
                        checksum += ord(i)
                    checksum = checksum % 256
                    if int(packetNo[0]) % 3 == 0:
                        checksum += 1

                    msg=f"[{str(packetNo[0])}] [{checksum}] {message}"
        ...
        try:
            if message:
                if message.lower() == ":quit":
                    ...
                    msg=f"[{str(packetNo[0])}] [{checksum}] {message}"
                    broadcast((msg).encode('utf-8'), "Sever")
                    packetNo[0]=str(int(packetNo[0])+1)
